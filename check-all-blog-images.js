...
function checkAllBlogImages() {
    ...
    files.forEach(file => {
        ...
        // 9. Check for specific malformed image patterns
        const specificBrokenPattern = /<picture><source srcset=" src="([^"]+)" type="image\/webp"><img src="([^"]+)" alt="([^"]+)" class="([^"]+)" loading="lazy"([^>]+)="" class="([^"]+)" loading="lazy"><\/picture>/g;
        const specificFixedPattern = '<picture>\n                    <source srcset="$1" type="image/webp">\n                    <img src="$2" \n                         alt="$3" \n                         class="$4" \n                         loading="lazy">\n                </picture>';

        if (content.match(specificBrokenPattern)) {
            content = content.replace(specificBrokenPattern, specificFixedPattern);
            hasChanges = true;
            console.log(`  ✅ Fixed specific malformed image pattern`);
        }

        // 10. Check for malformed canonical URLs that might affect page rendering
        ...
        if (content.match(malformedCanonicalPattern)) {
            ...
            hasChanges = true;
            console.log(`  ✅ Fixed malformed canonical URLs`);
        }
